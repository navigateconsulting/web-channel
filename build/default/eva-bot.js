...
class EvaBot extends LitElement {
    ...
    return html`
      ...
      <div class="main-div">
        ...
        <div class="chat-inner-scroll-div" id="chat-inner-div">
          ${this.chats.length ? html`
              ${this.chats.map(chat => html`
                ${chat.event === 'user' ? html`
    ...
    if (this.message.trim() !== '') {
      ...
      this.getBotResponse().then(responses => {
        if (responses.length) {
          ...
          this.chats.push({
            event: 'bot',
            text: 'Chat bot not Available. Please use UI Trainer to Train and Deploy new Model.'
          });
        }
  ...
  async getBotResponse() {
    const result = await fetch(this.serverUrl, {
      ...
      body: JSON.stringify({
        sender: this.conversation,
        message: this.message
      })
    }).catch(error => {
      return [{
        text: 'Unable to reach Rasa servers. Please check configuration.'
      }];
    });
